...
interface ModalUpdateValidatorProps {
    open: boolean
    setOpen: any,
    validators: any,
    setValidators: any
...
export default function ModalUpdateValidatorComponent({
    open,
    setOpen,
    validators,
    setValidators,
    validator
}: ModalUpdateValidatorProps) {
    ...
    async function onUpdate(event: any) {
        try {
            ...
            setValidators(validators.map((validatorItem: any) => {
                if (validatorItem.id === validator.id) {
                    validatorItem.name = name
                    validatorItem.param = param
                }
                        ...
                        <div className="inline-block align-bottom bg-white rounded-lg px-4 pt-5 pb-4 text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full sm:p-6">
                            <form className="space-y-8 divide-y divide-gray-200" onSubmit={event => onUpdate(event)}>
                                <div className="space-y-8 divide-y divide-gray-200 sm:space-y-5">
                                    <div>
                                        <div>
                                            <h3 className="text-lg leading-6 font-medium text-gray-900">Editar Validator</h3>
                                            <p className="mt-1 max-w-2xl text-sm text-gray-500">
                                                Para editar um validator entre com os dados abaixo.
                                            </p>
                                        </div>
